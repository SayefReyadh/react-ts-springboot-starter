...
export default function AsyncDemo() {
  ...
  const fireAndForget = async () => {
    ...
    try {
      const response = await fetch('/api/examples/multithreading/fire-and-forget', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskName: 'Demo Task', duration: 3 }),
      })
  ...
  const asyncWithResult = async () => {
    ...
    setLoading((prev) => ({ ...prev, [key]: true }))
    try {
      const response = await fetch('/api/examples/multithreading/with-result?seconds=2')
      const data = await response.json()
      addResult({
        taskId: 'with-result',
        status: 'completed',
  ...
  const parallelProcessing = async () => {
    ...
    setLoading((prev) => ({ ...prev, [key]: true }))
    try {
      const response = await fetch('/api/examples/multithreading/parallel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tasks: ['Task-A', 'Task-B', 'Task-C', 'Task-D'] }),
      })
      const data = await response.json()
      addResult({
        taskId: 'parallel',
  ...
  const processData = async () => {
    ...
    setLoading((prev) => ({ ...prev, [key]: true }))
    try {
      const response = await fetch('/api/examples/multithreading/process-data', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          items: ['Record-1', 'Record-2', 'Record-3', 'Record-4', 'Record-5'],
        }),
      })
      const data = await response.json()
      addResult({
  ...
  const getThreadPoolInfo = async () => {
    try {
      const response = await fetch('/api/examples/multithreading/thread-pool-info')
      const data = await response.json()
  ...
  const getScheduledInfo = async () => {
    try {
      const response = await fetch('/api/examples/multithreading/scheduled-info')
      const data = await response.json()
    ...
    <div style={{ padding: '20px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div>
          <div style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '20px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              >
                {loading.fireAndForget ? '⏳ Processing...' : '🚀 Fire and Forget'}
              </button>
              ...
              >
                {loading.parallel ? '⏳ Processing...' : '⚡ Parallel Processing'}
              </button>
              ...
              >
                {loading.processData ? '⏳ Processing...' : '📊 Process Data Async'}
              </button>
            </div>
          </div>
          ...
          <div style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '20px' }}>
            ...
            {threadPoolInfo && (
              <div style={{ marginTop: '15px', padding: '10px', backgroundColor: '#f5f5f5', borderRadius: '4px', fontSize: '13px' }}>
            ...
            {scheduledInfo && (
              <div style={{ marginTop: '15px', padding: '10px', backgroundColor: '#fff3e0', borderRadius: '4px', fontSize: '13px' }}>
              ...
              results.map((result, idx) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <strong style={{ color: '#2196F3' }}>{result.taskId}</strong>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', fontSize: '14px' }}>
          <div>
            ...
            <p style={{ margin: '5px 0', color: '#666' }}>
              Execute task asynchronously without waiting for result. Perfect for logging, notifications, emails.
            </p>
          </div>
          <div>
            <strong>📥 Async with Result (CompletableFuture)</strong>
            <p style={{ margin: '5px 0', color: '#666' }}>
              Execute async and return result when done. Use for API calls, data processing.
            </p>
          </div>
          <div>
            <strong>⚡ Parallel Processing (CompletableFuture.allOf)</strong>
            <p style={{ margin: '5px 0', color: '#666' }}>
              Run multiple tasks concurrently and combine results. Great for batch operations.
            </p>
          </div>
          <div>
      ...
      <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#fff3e0', borderRadius: '8px' }}>
        ...
        <ul style={{ fontSize: '14px', lineHeight: '1.8', margin: '10px 0' }}>
          <li><strong>@EnableAsync:</strong> Enables async method execution</li>
          <li><strong>ThreadPoolTaskExecutor:</strong> Core: 5, Max: 10, Queue: 100</li>
          <li><strong>@Async:</strong> Methods run in separate threads from configured pool</li>
          <li><strong>CompletableFuture:</strong> Handles async results and composition</li>
        </ul>
      </div>
    </div>
  )
